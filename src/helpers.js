...
const REGEX_FILE_SRC_HARDCODED = new RegExp(`(href|src)="${domain}.+(pdf|jpg|png|jpeg|docx|doc|gif|xsl)"`, 'g')
...
const createDownloadablePaths = () => {
    absolutePaths = files.map((file) => {
        return file
            .replace(`href="/`, `${domain}/`).replace('href="', '')
            .replace(`src="/`, `${domain}/`).replace('src="', '')
            .replace('"', '')
    })
}
...
const stripInlineStyles = (standAlone = false) => {
    ...
    let output = standAlone === false ?
        fs.readFileSync("../output.html", 'utf-8').replaceAll(REGEX_INLINE_STYLE, '').replaceAll(REGEX_CLASS, '') :
        fs.readFileSync("../input.html", 'utf-8').replaceAll(REGEX_INLINE_STYLE, '').replaceAll(REGEX_CLASS, '')
    fs.writeFileSync('../output.html', output)
}
